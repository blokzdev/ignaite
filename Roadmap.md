...
| AF-2  | Capability `level` backfill campaign — `/level-capabilities` routine + 43-cat sweep; **1015/1015 leveled**   | #359–362,#364 | ✅     |
| AF-3  | Level-aware capability UI — primary (filled) vs secondary (ghost) chips on `/apps/[slug]`, legend + a11y     | #363          | ✅     |
...
- **Chunk AE — Recipe routes + SEO ✅ (#354)** — `/recipes` hub + `/recipes/<slug>` detail (SSG,
  pure RSC, **0 B route JS** like `/compare`); the ordered-step walkthrough (linked apps +
  family-toned capability chips + action/rationale), prerequisites, references, and a
  `status:"stale"` caveat banner. `HowTo` + `BreadcrumbList` JSON-LD with `#step-N` anchors
  (semantic/LLM value; Google deprecated the rich result — thesis is internal-link densification +
  llms/feed, no fabricated schema fields). Per-recipe Satori OG (`renderRecipeOgImage`, guarded
  endpoint). Per-app "Used in N recipes" rail (`recipesUsingApp`). Recipes wired into `sitemap.ts`,
  `feed.json` (`_ignaite.recipes` graph payload), `llms.txt`/`llms-full.txt` (linearized
  walkthroughs), and the ⌘K palette (new slim `recipes-search.json` — client stays corpus-free).
...
- **Chunk AF-2 — `level` backfill campaign ✅ COMPLETE (#359–362, #364)** (founder-directed, **after AH**) —
  populated `capabilities[].level` (`primary` = the 1–2 headline jobs the app is built around; `secondary`
  = supporting) across the **entire corpus: 1015/1015 capped active listings, 0 unleveled**, ~45% primary
  (1637 primary / 2029 secondary), via a **full web-verify** AB-style Ultracode fan-out. Enrichment only —
  no `lastVerifiedAt` / `changelog` (AB precedent). Mechanism: the **`/level-capabilities [category]`
  routine** (`.claude/commands/`) processes **one category per run** — fan-out leveling agents →
  adversarial **primary-inflation** audit → apply → PR — and **no-ops now that every listing is leveled**
  (then `/add-app` + `/audit-directory`, both now level-aware, keep the axis current). Shipped as the
  `agent` pilot that locked the rubric (#359), three calibration categories (#360–362), then one resumable
  bulk workflow that leveled the remaining 35 categories + a re-run of 4 (#364). Sharpens the
  substitution engine (AF-1) and is surfaced to humans by AF-3.
- **Chunk AF-3 — level-aware capability UI ✅ (#363)** — the human-facing half of AF-2 on `/apps/[slug]`:
  within each capability-family row, **primary** ("built for this") chips read solid + tone-coloured and
  lead; **secondary** ("can also do") recede as ghost outline chips. A leading dot (filled vs hollow) +
  per-chip `sr-only` level + a legend carry the distinction without relying on colour; `note` qualifiers
  render inline. `groupCapabilityEntriesByFamily()` orders primary-first. Graceful: unleveled listings
  render exactly as before, so it shipped safely mid-campaign. Pure RSC, 0 added client JS.
