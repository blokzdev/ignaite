...
- [ ] **[verify]** Velite (0.3.1) runs in **non-strict** mode: a schema-invalid `data/apps/*.json` or `data/sponsored/*.json` is **silently excluded** from the generated output (validation runs — the bad doc never ships — but the build stays green with no error). This contradicts the "precise per-file error" expectation in CLAUDE.md §5 + the authoring routines: a routine that writes a malformed entry sees it silently vanish from the directory rather than a hard failure. Fix: set `strict: true` in `velite.config.ts` so validation errors throw. Deferred from the sponsored PR because it's a build-pipeline/config change (CLAUDE.md §11) affecting the whole data layer, not just sponsored — wants its own small PR + a re-validation of all ~125 existing apps against the stricter gate.
