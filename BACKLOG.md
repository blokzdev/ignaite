...
- [ ] **[debt]** The SW precache can serve a **stale `/offline` shell across deploys**: `PRECACHE_URLS` (`public/sw.js` — `/`, `/offline`, manifest; cache `ignaite-shell-v1`) is written only at SW install, and `sw.js`'s bytes never change between deploys, so the browser never reinstalls the SW and the precache never refreshes. The cached `/` self-heals (network-first navigations `cache.put` it on every online visit), but `/offline` is never navigated to online, so its cached HTML keeps referencing hashed `/_next/static` chunks that disappear on later deploys → the offline fallback can silently break over time (its JS 404s; static markup still renders). Old hashed assets also accumulate in the SWR cache with no eviction. Unblock: embed a build identifier in `sw.js` at build time (forces a byte-diff → reinstall → fresh precache + the `CACHE` bump cleans old entries) or periodically re-fetch `/offline` from the activate handler — small dedicated PR; pairs with the Serwist item below if that lands first.
