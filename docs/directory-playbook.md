...
| ------------------------------------------------------------------ | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`/add-app <name \| url \| list>`**                               | a human supplies names          | Dedup (slug + name + primary domain) → web-research → author a schema-valid `App` entry → validate → report.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **`/discover-apps [focus]`**                                       | autonomous (good for schedules) | Finds net-new apps not yet listed (dedups by **slug + name + primary domain** against `main` **and** open discovery PRs; biases toward the thinnest under-covered categories, never padding sparse ones), authors the worthy ones, and **opens a PR**. The unattended counterpart to `/add-app`. Run manually it just opens the PR; the **scheduled routine is fire-and-forget — it enables auto-merge and ends, and GitHub lands the PR server-side once CI is green** (see [Scheduling](#scheduling-claude-code-routines)).                                               |
...
| **`/level-capabilities [category]`**                               | autonomous (good for schedules) | **Finite campaign** (Chunk AF-2): backfills each capability's **`level`** (`primary` = the 1–2 headline jobs the app is built around, `secondary` = supporting) across one **category** per run, fanning out web-verified leveling agents + an adversarial primary-inflation audit, then **opens a PR (auto-merges on green CI)**. Enrichment-only — touches only `capabilities[].level` (+ rare web-confirmed cap adds), never `lastVerifiedAt`/`changelog`. **No-ops once every active listing is leveled** (then `/add-app` + `/audit-directory` keep the axis leveled). |

These commands are defined in `.claude/commands/` and committed to the repo, so anyone running Claude
Code here can invoke them. They encode the same flow we run manually. (The recipe routines
`/author-recipes` + `/audit-recipes` live there too — see their command files.)
...
- **Archive, don't delete.** Discontinued apps get `status: "archived"` (kept as record, hidden from
  the default browse).
- **No duplicates — match by name + domain, not just slug.** Before authoring, `/add-app` and
  `/discover-apps` scan each candidate's **normalized name** and **primary-link domain** against the
  whole directory (and open discovery PRs), because the same app re-added under a different slug
  (`bland` vs `bland-ai`) defeats a slug-only check. A name **or** domain hit means it's already listed
  (a domain hit with a genuinely different product name can be a legitimate sibling on a shared company
  domain — `langchain`/`langgraph`). Backstop: `velite build --strict` **hard-fails** on a duplicate
  listing (same normalized name + primary domain), so a missed dup fails CI. To merge a confirmed
  duplicate, keep the canonical (older) entry and redirect the other `/apps/<slug>` in `next.config.ts`.
...
- **Capability-leveling campaign** (Chunk AF-2 — finite; retire the schedule once it drains)
  > Run `/level-capabilities`. Pick the category with the most unleveled capabilities, fan out
  > web-verified leveling agents (`primary` = the 1–2 headline jobs the app is built around;
  > `secondary` = supporting; default `secondary` when uncertain), run the adversarial primary-inflation
  > audit, apply, validate locally (`pnpm velite` + `typecheck` + `lint` + `build`), and open a PR —
  > enrichment-only (no `lastVerifiedAt`/`changelog`). Then **enable squash auto-merge and END the run**
  > — do NOT wait for CI or schedule a check-in. GitHub merges it once CI is green. Once every active
  > listing is fully leveled, do nothing — the campaign is complete; **disable this schedule** then.
