...
| **`/discover-apps [focus]`**                                       | autonomous (good for schedules) | Finds net-new apps not yet listed (dedups against `main` **and** open discovery PRs), authors the worthy ones, and **opens a PR**. The unattended counterpart to `/add-app`.                |
